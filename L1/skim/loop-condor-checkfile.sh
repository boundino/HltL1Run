...
if [[ $# -ne 4 ]]; then
    echo "usage: ./loop-condor-checkfile.sh [input dir] [output dir] [max jobs] [log dir]"
    exit 1
fi
...
Initialdir   = $PWD/
Notification = Error
Executable   = $PWD/loop-skim.sh
Arguments    = $INFILE $DESTINATION ${NAME}_${infn}.root $PROXYFILE
GetEnv       = True
...
transfer_input_files = skim.exe,/tmp/$PROXYFILE
